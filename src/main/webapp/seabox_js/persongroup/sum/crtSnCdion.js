...
function initTagRadio(){
    $(".trd_tree_div").click(function(){
        ...
        $(radioLst).on('ifClicked', function(event){
            ...
            //每次选中都会使右侧的标签树发生变化,所以需要动态计算圈定人群数
            var checkTags = sumTagToObj();
            // alert(JSON.stringify(checkTags));

            submitJsonStr2Controller("/screen/sum/sumPCnt.do",saveBackFunction,checkTags);
...
function addTagGrp(){
    ...
    var newTagGrp = $("<div id='tag_grp_seq_3' class='allow_edit'></div>");
...
function saveSnCdion(saveBtn) {
    ...
    sendData["screeningNm"] = $("#screeningNm").val();
    sendData["tagData"] = tagData;
    submitJsonStr2Controller("/screen/sum/saveScreening.do",saveBackFunction,sendData);
