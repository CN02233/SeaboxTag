...
(function ($) {
    $.fn.extendPagination = function (options) {
        ...
        if (defaults.limit == '') {
            defaults.limit = '9';
        } else if (Number(defaults.limit) <= 0)defaults.limit = '9';
        var currPage = Number(defaults.currPage), totalCount = Number(defaults.totalCount), showPage = Number(defaults.showPage),
            limit = Number(defaults.limit), totalPage = Math.ceil(totalCount / limit);
        if (totalPage > 0) {
            var html = [];
            html.push('<ul class="pagination">');
            html.push('<li class="page_previous"><a href="javascript:void(0);">&lt;</a></li>');
            // html.push('<li class=""><a href="javascript:void(0);">&lt;</a></li>');
            html.push('<li class="disabled hidden"><a href="javascript:void(0);">...</a></li>');
            if (totalPage <= showPage) {
                for (var i = 1; i <= totalPage; i++) {
                    if (i == currPage) html.push('<li class="active"><a href="javascript:void(0);">' + i + '</a></li>');
                    else html.push('<li><a href="javascript:void(0);">' + i + '</a></li>');
                }
            } else {
                for (var j = 1; j <= showPage; j++) {
                    if (j == currPage) html.push('<li class="active"><a href="javascript:void(0);">' + j + '</a></li>');
                    else html.push('<li><a href="javascript:void(0);">' + j + '</a></li>');
                }
            }
            html.push('<li class="disabled hidden"><a href="javascript:void(0);">...</a></li>');
            html.push('<li class="next"><a href="javascript:void(0);">&gt;</a></li></ul>');
            $(this).html(html.join(''));
            if (totalPage > showPage) $(this).find('ul.pagination li.next').prev().removeClass('hidden');

            var pageObj = $(this).find('ul.pagination'), preObj = pageObj.find('li.page_previous'),
                currentObj = pageObj.find('li').not('.page_previous,.disabled,.next'),
